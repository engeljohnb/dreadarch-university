...
func init_player():
	...
	hud.set_treasure(0)
	if not ItemCollection.item_collected.is_connected(player.on_item_collected):
		ItemCollection.item_collected.connect(player.on_item_collected)
	if not ItemCollection.scroll_fragment_translated.is_connected(player.on_scroll_fragment_translated):
		ItemCollection.scroll_fragment_translated.connect(player.on_scroll_fragment_translated)
...
func get_all_save_filenames():
	...
	if dir == null:
		...
		da.make_dir("user://SaveFiles")
		dir = DirAccess.open("user://SaveFiles/")
...
func update_save_data():
	...
	_save.completed_tutorial_prompts = Tutorial.get_completed_tutorial_prompts()
...
func load_hud(player_data):
	...
	hud.set_treasure(player.inventory[ItemCollection.TREASURE])
...
func load_game(filename = "user://SaveFiles/save.da"):
	...
	Tutorial.load_completed_tutorial_prompts(_save.completed_tutorial_prompts)
